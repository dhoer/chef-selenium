...
describe 'selenium::server' do
  context 'windows' do
    ...
    it 'downloads jar' do
      expect(chef_run).to create_remote_file('C:/selenium/server/selenium-server-standalone-2.46.0.jar')
    end

    it 'creates link to jar' do
      expect(chef_run).to create_link('C:/selenium/server/selenium-server-standalone.jar').with(
        to: 'C:/selenium/server/selenium-server-standalone-2.46.0.jar'
      )
    end
  end
  ...
  context 'linux' do
    ...
    it 'downloads jar' do
      expect(chef_run).to create_remote_file('/usr/local/selenium/server/selenium-server-standalone-2.46.0.jar')
    end

    it 'creates link to jar' do
      expect(chef_run).to create_link('/usr/local/selenium/server/selenium-server-standalone.jar').with(
        to: '/usr/local/selenium/server/selenium-server-standalone-2.46.0.jar'
      )
    end
  end
